#include "corecast_optix/corecast_optix_program_registry.hpp"
#include <corecast_optix/corecast_optix_types.hpp>

#include <optix_stubs.h>

#include <sstream>
#include <stdexcept>

namespace corecast::optix {

CoreCastOptixProgramRegistry::CoreCastOptixProgramRegistry(std::shared_ptr<CoreCastOptixContext> context)
    : context_(context) {}

CoreCastOptixProgramRegistry::~CoreCastOptixProgramRegistry() {
  for (auto& program_group : program_groups_) {
    optixProgramGroupDestroy(program_group.second);
  }
  program_groups_.clear();
}

void CoreCastOptixProgramRegistry::register_program(const CoreCastProgram& program, CoreCastOptixModule& module) {
  program_descriptions_[program.name] = program;

  OptixProgramGroupDesc& description = program_descriptions_[program.name].desc;

  if (description.kind == OPTIX_PROGRAM_GROUP_KIND_RAYGEN) {
    description.raygen.module = module.module_  ;
  } else if (description.kind == OPTIX_PROGRAM_GROUP_KIND_MISS) {
    description.miss.module = module.module_;
  } else if (description.kind == OPTIX_PROGRAM_GROUP_KIND_HITGROUP) {
    description.hitgroup.moduleCH = module.module_;
  } else {
    throw std::runtime_error("Invalid program group kind");
  }

  OPTIX_CHECK_LOG(optixProgramGroupCreate(context_->device_context_, &program_descriptions_[program.name].desc, 1,
                                          &program_descriptions_[program.name].options, LOG, &LOG_SIZE,
                                          &program_groups_[program.name]));
}

OptixProgramGroup CoreCastOptixProgramRegistry::get_program_group(const std::string& name) const {
  return program_groups_.at(name);
}

std::vector<OptixProgramGroup> CoreCastOptixProgramRegistry::get_program_groups(
    const std::vector<std::string>& names) const {
  std::vector<OptixProgramGroup> groups;
  for (const auto& name : names) {
    groups.push_back(program_groups_.at(name));
  }
  return groups;
}

}  // namespace corecast::optix
