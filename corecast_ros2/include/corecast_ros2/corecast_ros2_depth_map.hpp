...
public:
  ...
  void process_depthmap(const sensor_msgs::msg::PointCloud2::SharedPtr msg)
  {
    ...
    depth_map_processor_->setup_depth_map_processing(
    point_cloud_, depth_map_width_, depth_map_height_, camera_frame_data, 0.01f);
...
private:
                              ...
                              corecast::optix::CameraFrameData& camera_frame_data)
  {
    try {
      ...
      camera_frame_data.sensor_origin = make_float3(
      ...
      camera_frame_data.sensor_x_axis = make_float3(
        static_cast<float>(rot[0][0]), static_cast<float>(rot[1][0]), static_cast<float>(rot[2][0]));
      camera_frame_data.sensor_y_axis = make_float3(
        static_cast<float>(rot[0][1]), static_cast<float>(rot[1][1]), static_cast<float>(rot[2][1]));
      camera_frame_data.sensor_z_axis = make_float3(
        static_cast<float>(rot[0][2]), static_cast<float>(rot[1][2]), static_cast<float>(rot[2][2]));
      return true;
    } catch (const tf2::TransformException& ex) {
      RCLCPP_WARN_THROTTLE(node_.get_logger(), *node_.get_clock(), 2000,
                           "TF lookup failed for camera_frame='%s' -> cloud_frame='%s': %s",
                           camera_frame_.c_str(), msg.header.frame_id.c_str(), ex.what());
      return false;
    }
  }
