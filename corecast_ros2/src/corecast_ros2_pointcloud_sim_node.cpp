...
public:
  ...
  : Node("corecast_ros2_pointcloud_sim_node"), phase_(0.0)
  {
    ...
    RCLCPP_INFO(get_logger(), "PointCloud2 simulator publishing to /point_cloud");
  }
...
private:
  void publish_pointcloud()
  {
    ...
    const float yaw = 0.75f * std::sin(t * 0.7f);
    const float pitch = 0.55f * std::cos(t * 0.5f);
    ...
    for (uint32_t ring = 0; ring < kHeight; ++ring) {
      ...
      for (uint32_t i = 0; i < kWidth; ++i, ++x_it, ++y_it, ++z_it, ++intensity_it, ++ring_it, ++time_it) {
        ...
        const float wave1 = std::sin(azimuth * 6.0f + t * 2.2f);
        const float wave2 = std::cos(elevation * 5.0f - t * 1.7f);
        const float wave3 = std::sin((azimuth + elevation) * 4.0f + t * 1.3f);
        const float shell_breath = 1.0f + 0.16f * std::sin(t * 1.1f);
        ...
        x += 1.8f * std::sin(t * 0.9f);
        y += 1.4f * std::cos(t * 1.3f);
        z += 1.2f * std::sin(t * 1.6f);
        ...
        *y_it = y_pitch;
        *z_it = z_yaw;
        *intensity_it = 120.0f + 100.0f * std::sin(azimuth * 2.0f + elevation * 3.0f + t * 2.4f);
